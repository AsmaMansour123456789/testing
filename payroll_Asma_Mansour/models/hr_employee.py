...
class HrEmployee(models.Model):
    ...
    def onchange_echellee(self):
        ...
        notification_ids = []
        print("user_id",self.Approvers_echelle.user_id)
        # user = self.env['res.users'].browse(self.parent_id.user_id.id)

        if self.Approvers_echelle.user_id.partner_id.id:
            notification_ids.append(
                    (0, 0, {'res_partner_id': self.Approvers_echelle.user_id.partner_id.id, 'notification_type': 'inbox'}))
            print("notification_ids",notification_ids)

            self.env['mail.message'].create({'message_type': 'notification',
                                             'subtype_id': self.env.ref("mail.mt_comment").id,
                                             'body': "Echelle de l'employé " +employee.name+" augmente de 1:",
                                             'subject': "Merci d'approver l'echelle "+str(employee.name),
